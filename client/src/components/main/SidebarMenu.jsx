...
const SidebarMenu = ({ userData }) => {
    ...
    const handleCurrentUrlChange = () => {
      ...
      if (
        (currentURL?.split('/')[2] === userData._id &&
          currentURL?.split('/')[3] === 'friends' &&
          currentPage !== 'friends') ||
        (currentURL?.split('/')[2] === userData._id &&
          currentURL?.split('/')[3] === 'friend-requests' &&
          currentPage !== 'friends')
      ) {
        return setCurrentPage('friends');
      }
  ...
  if (
    localStorage.getItem('currentURL') === '/login' ||
    localStorage.getItem('currentURL') === '/signup'
  ) {
    return null;
  }
  ...
  return (
    <div className="sidebar-menu">
      ...
      <Link
        to={`/profile/${userData._id}/friends`}
